...
const photoApi = {
  ...
  // GET 요청URL HTTP/1.1
  fetch: () =>
    axios.get<PhotoItemResponse[]>(`${"http://localhost:8080"}/photos`),

  fetchPaging: (page: number, size: number) =>
    axios.get<PhotoPagingReponse>(
      `${"http://localhost:8080"}/photos/paging?page=${page}&size=${size}`
    ),
  ...
  remove: (id: number) =>
    axios.delete<boolean>(`${"http://localhost:8080"}/photos/${id}`),
