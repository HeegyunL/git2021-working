...
public class PhotoRepositorySupport {
	...
	public List<Photo> searchByKeyword(String keyword) {
		return query.selectFrom(photo).where(photo.title.containsIgnoreCase(keyword)
				.or(photo.description.containsIgnoreCase(keyword)).or(photo.fileName.containsIgnoreCase(keyword)))
				.orderBy(photo.id.desc()).fetch();
	}
}